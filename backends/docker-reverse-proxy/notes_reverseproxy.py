...
PROJECT_STRUCTURE = {
    "Dockerfile": "multi-stage build, non-root user, no EXPOSE",
    "nginx/default.conf": "upstream + proxy headers config",
    "docker-compose.yml": "nginx (ports 80) + app (expose only, no host port)",
    "app/main.py": "FastAPI showing forwarded headers",
}

KEY_INSIGHT = (
    "nginx has ports: [\"80:80\"] — reachable from host. "
    "app has expose: [\"8000\"] — internal only. "
    "curl localhost works. curl localhost:8000 is connection refused."
)
