...
ISSUES = {
    "ISSUE 1": "Unpinned base image (python:3.12 not python:3.12.X-slim-bookworm)",
    "ISSUE 2": "Secrets in ARG/ENV (visible in docker history / docker inspect)",
    "ISSUE 3": "Full base image (not slim — large attack surface)",
    "ISSUE 4": "COPY . . (copies .env, secrets/, .git/ into image)",
    "ISSUE 5": "No pip cache strategy",
    "ISSUE 6": "No USER instruction — runs as root (uid 0)",
}

FIXES = {
    "FIX 1": "Pinned to python:3.12-slim-bookworm (version + OS variant explicit)",
    "FIX 2": "Secrets via --mount=type=secret (build) or compose secrets: (runtime)",
    "FIX 3": "Slim base — no compiler, no curl, no git",
    "FIX 4": "COPY app/ — only app code; .dockerignore excludes secrets/",
    "FIX 5": "BuildKit cache mount for pip downloads",
    "FIX 6": "Non-root USER (uid 1001) with explicit chown",
}


def print_comparison():
    print("Insecure → Secure comparison:")
    for (issue_key, issue_desc), (fix_key, fix_desc) in zip(ISSUES.items(), FIXES.items()):
        print(f"\n  [{issue_key}] {issue_desc}")
        print(f"  [{fix_key}]   {fix_desc}")
