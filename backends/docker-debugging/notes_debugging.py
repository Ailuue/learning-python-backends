...
DEBUGGING_WORKFLOW = [
    ("docker compose ps",            "Is the container up, exited, or unhealthy?"),
    ("docker compose logs <svc>",    "What did the process print? Any crash output?"),
    ("docker compose exec <svc> sh", "Get inside and inspect env, files, network"),
    ("docker inspect <container>",   "Verify ports, mounts, env vars, health history"),
    ("docker stats",                 "Is the container being OOM-killed or throttled?"),
]

INSPECT_FORMATS = {
    "State":       "{{.State.Status}} exit={{.State.ExitCode}} oom={{.State.OOMKilled}}",
    "Env vars":    "{{range .Config.Env}}{{println .}}{{end}}",
    "Ports":       "{{json .NetworkSettings.Ports}}",
    "Mounts":      "{{range .Mounts}}{{.Source}} → {{.Destination}}{{println}}{{end}}",
...
def print_workflow():
    ...
    for i, (cmd, purpose) in enumerate(DEBUGGING_WORKFLOW, 1):
        print(f"  {i}. {cmd:<40} {purpose}")


def print_inspect_formats():
    print("docker inspect --format patterns:")
    for label, fmt in INSPECT_FORMATS.items():
